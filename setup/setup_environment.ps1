# =============================================================================
# CVE-2021-4034 (PwnKit) Lab Environment Setup Script - PowerShell Version
# =============================================================================

# Configuration
$VM_KALI_NAME = "Kali_Attacker_PwnKit"
$VM_VULN_NAME = "Ubuntu_Vulnerable_PwnKit"
$VM_OS_TYPE = "Linux_64"
$VM_RAM = 2048
$VM_VRAM = 128
$VM_CPUS = 2
$NETWORK_NAME = "vboxnet0"

# Paths
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Definition
$VDI_KALI_PATH = Join-Path $ScriptDir "vbox_disks\kali.vdi"
$VDI_VULN_PATH = Join-Path $ScriptDir "vbox_disks\ubuntu.vdi"

# Helper functions
function Log-Info { Write-Host "[INFO] $args" -ForegroundColor Green }
function Log-Warn { Write-Host "[WAARSCHUWING] $args" -ForegroundColor Yellow }
function Log-Error { Write-Host "[FOUT] $args" -ForegroundColor Red }

function Check-Prerequisites {
    Log-Info "Controleren van vereisten..."

    if (-not (Get-Command VBoxManage.exe -ErrorAction SilentlyContinue)) {
        Log-Error "VirtualBox is niet geïnstalleerd of VBoxManage is niet in PATH."
        exit 1
    }

    if (-not (Test-Path $VDI_KALI_PATH)) {
        Log-Error "Kali VDI bestand niet gevonden op $VDI_KALI_PATH"
        exit 1
    }

    if (-not (Test-Path $VDI_VULN_PATH)) {
        Log-Error "Ubuntu VDI bestand niet gevonden op $VDI_VULN_PATH"
        exit 1
    }

    Log-Info "Alle vereisten zijn voldaan."
}

function Create-HostOnlyNetwork {
    Log-Info "Aanmaken van host-only netwerk..."

    $ifExists = VBoxManage list hostonlyifs | Select-String $NETWORK_NAME

    if (-not $ifExists) {
        VBoxManage hostonlyif create | Out-Null
        Log-Info "Host-only netwerk aangemaakt: $NETWORK_NAME"
    } else {
        Log-Info "Host-only netwerk $NETWORK_NAME bestaat al."
    }
}

function Cleanup-VM($vmName) {
    $exists = VBoxManage list vms | Select-String $vmName

    if ($exists) {
        Log-Info "Opruimen van bestaande VM: $vmName"
        VBoxManage controlvm $vmName poweroff 2>$null
        VBoxManage unregistervm $vmName --delete 2>$null
        Start-Sleep -Seconds 2
    }
}

function Create-VM($vmName, $vdiPath, $osType) {
    Log-Info "Aanmaken van VM: $vmName"

    Cleanup-VM $vmName

    VBoxManage createvm --name $vmName --ostype $osType --register

    VBoxManage modifyvm $vmName `
        --memory $VM_RAM `
        --cpus $VM_CPUS `
        --vram $VM_VRAM `
        --graphicscontroller vmsvga `
        --audio none `
        --clipboard-mode bidirectional `
        --nic1 hostonly `
        --hostonlyadapter1 $NETWORK_NAME `
        --nicpromisc1 allow-all

    VBoxManage storagectl $vmName `
        --name "SATA Controller" `
        --add sata `
        --controller IntelAhci `
        --portcount 1 `
        --bootable on

    VBoxManage storageattach $vmName `
        --storagectl "SATA Controller" `
        --port 0 `
        --device 0 `
        --type hdd `
        --medium $vdiPath

    Log-Info "VM $vmName succesvol aangemaakt."
}

# =============================================================================
# Main Script
# =============================================================================

Write-Host "Start CVE-2021-4034 (PwnKit) Lab Omgeving Setup"
Write-Host "===================================================="

Check-Prerequisites
Create-HostOnlyNetwork
Create-VM -vmName $VM_KALI_NAME -vdiPath $VDI_KALI_PATH -osType "Debian_64"
Create-VM -vmName $VM_VULN_NAME -vdiPath $VDI_VULN_PATH -osType "Ubuntu_64"

Log-Info "Starten van VM's..."
VBoxManage startvm $VM_KALI_NAME --type gui
VBoxManage startvm $VM_VULN_NAME --type gui

Write-Host "===================================================="
Write-Host "Setup succesvol voltooid!"
Write-Host ""
Write-Host "Volgende stappen:"
Write-Host "1. Log in op $VM_VULN_NAME (Inloggegevens: osboxes/osboxes.org)"
Write-Host "2. Maak SSH gebruiker aan:"
Write-Host "   sudo useradd -m tester"
Write-Host "   echo 'tester:testerpwd' | sudo chpasswd"
Write-Host "   sudo usermod -aG sudo tester"
Write-Host "3. Configureer SSH:"
Write-Host "   sudo apt update"
Write-Host "   sudo apt install -y openssh-server"
Write-Host "   sudo systemctl enable --now ssh"
Write-Host "   sudo ufw allow 22/tcp"
Write-Host "4. Log in op $VM_KALI_NAME (Inloggegevens: osboxes/osboxes)"
Write-Host "5. Ga verder naar de exploit gids in docs/exploitguide.md"
Write-Host "===================================================="

exit 0
